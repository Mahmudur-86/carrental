 ...
 <img src={assets.search_icon} alt="search" />
    ...
    <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
      ...
      <button onClick={()=> isOwner ?  navigate('/owner') : 
        changeRole() } className='cursor-pointer'>{isOwner ?  "Dashboard" : 'List cars' }</button>


      <button onClick={()=> {user ? logout() : setShowLogin(true)} } className='cursor-pointer px-9 py-2 bg-primary hover:bg-primary-dull transition-all text-white rounded-lg'>{ user ? 'Logout' : 'Login' } 
