...
<div className='flex items-center justify-center w-10 h-10 rounded-full bg-cyan-950'>
{/* items */}
...
{/* recent booking */}
    <div className='p-4 md:p-6  border border-b-amber-600 border-indigo-700 rounded-md max-w-lg w-full'>
<h1 className='text-lg font-medium'>Recent Booking</h1>
<p className='text-amber-950'> Latest customer bookings</p>
{data.recentBookings.map((booking, index)=>(
<div key={index} className='mt-4 flex items-center justify-between'>
<div className='flex items-center gap-2'>
<div className='hidden md:flex items-center justify-center w-12 h-12 rounded-full bg-amber-600'>
    <img src={assets.listIconColored} alt="" className='h-5 w-5' />
</div>
<div>
    <p>{booking.car.brand}  {booking.car.model}</p>
    <p className='text-sm text-lime-500'>{booking.createdAt.split('T')[0]}</p>
</div>
</div>
...
<p className='px-3 py-0.5 border border-borderColor rounded-full text-sm'>{booking.status}</p>
    ...
    {/* monthly revenue */}
    <div className='p-5 md:p-8 mb-6 border border-e-teal-800 rounded-md w-full md:max-w-xs'>
